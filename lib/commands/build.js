...
async function buildWasm(projectPath, tmpDir) {
  ...
  const wasmExists = await Promise.all(
    wasmFiles.map((file) => fs.pathExists(file))
  );
  if (wasmExists.every((exists) => exists)) {
    console.log(chalk.green("Using existing WASM files"));
    // Copy existing WASM files to temp directory
    await Promise.all(
      wasmFiles.map((file) =>
        fs.copyFile(file, path.join(tmpDir, path.basename(file)))
      )
    );
    return {
      wasmPath: path.join(tmpDir, `${projectName}_bg.wasm`),
      jsPath: path.join(tmpDir, `${projectName}.js`),
    };
  }
  ...
  console.log(chalk.gray(`Target directory: ${process.env.CARGO_TARGET_DIR}`));
  ...
  try {
    ...
    // Use target web for wasm-bindgen
    const { stdout: bindgenStdout, stderr: bindgenStderr } = await execAsync(
      `wasm-bindgen ${wasmPath} --out-dir ${tmpDir} --target web`,
      { cwd: projectPath }
    );
...
async function build() {
  ...
  try {
    ...
    console.log(chalk.cyan(`Using build directory: ${buildDir}`));

    // Check for stored schema first
    const hasStoredSchema = await handleSchema(process.cwd());
    if (!hasStoredSchema) {
      console.log(
        chalk.yellow("No stored schema found, generating new schema...")
      );
      ...
      console.log(chalk.green("Using stored schema files"));
    }
    ...
    if (schemaFiles.length === 0) {
      throw new Error("No schema files found after schema generation/copy");
    }
    console.log(chalk.cyan(`Found ${schemaFiles.length} schema files`));

    // Build and optimize WASM
    spinner.text = chalk.cyan("Building WebAssembly (0%)");
    const { wasmPath, jsPath } = await buildWasm(process.cwd(), buildDir);

    // Only optimize if we just built the WASM
    if (!wasmPath.includes("_bg.wasm")) {
    ...
    // Stop spinner before prompting for seed
    spinner.stop();
    console.log(chalk.cyan("\nLTO Account Setup"));
    ...
    // Restart spinner after seed is entered
