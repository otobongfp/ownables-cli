...
async function buildWasm(projectPath, spinner) {
  ...
  try {
    ...
    if (!fs.existsSync(wasmPath)) {
      throw new Error(
        `WASM file not found at ${wasmPath}. Build may have failed.`
      );
    }
...
async function createPackage(
  ...
  await Promise.all([
    ...
    (async () => {
      ...
      // Copy to both root and images directory
      await fs.copy(imagePath, path.join(outputDir, imageFile));
      await fs.copy(imagePath, path.join(outputDir, "images", imageFile));
      spinner.text = chalk.cyan(
        `Processing files (${Math.round(
          (completedSteps / totalSteps) * 100
        )}%) - Creating thumbnail`
    ...
    (async () => {
      ...
      indexHtmlContent = indexHtmlContent
        ...
        // Handle both direct image names and images/image names
        .replace(/src=["'](?:images\/)?([^"']+)["']/g, (match, imageName) => {
          // If the image exists in the images directory, use the full path
          const imagePath = path.join(projectPath, "assets/images", imageName);
          if (fs.existsSync(imagePath)) {
            return `src="images/${imageName}"`;
          }
          // Otherwise, use the direct image name
          return `src="${imageName}"`;
        })
        // Remove trailing slashes from src attributes
        .replace(/src="([^"]+)\/"/g, 'src="$1"')
