...
async function createPackage(projectPath, outputDir, metadata, spinner) {
  ...
  // Copy WASM and JS files in parallel
  ...
  // Copy schema files in parallel with package.json creation
  spinner.text = "Copying schema files and creating package.json...";
  await Promise.all([
  ...
  await Promise.all(
    files.map(async (file) => {
      ...
      if (stats.isDirectory()) {
        await addDirToZip(zip, filePath, file);
      } else {
...
// Helper function to add directory contents to ZIP in parallel
async function addDirToZip(zip, dirPath, zipPath) {
  ...
  await Promise.all(
    files.map(async (file) => {
      ...
      if (stats.isDirectory()) {
        await addDirToZip(zip, filePath, relativePath);
      } else {
        const content = await fs.readFile(filePath);
