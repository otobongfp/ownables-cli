...
async function buildWasm(projectPath, spinner) {
  try {
    ...
    if (wasmStdout) console.log(chalk.gray(wasmStdout));
    if (wasmStderr) console.error(chalk.yellow(wasmStderr));
    console.log(chalk.green("WebAssembly module built successfully"));
    ...
    const { stdout: bindgenStdout, stderr: bindgenStderr } = await execAsync(
      "wasm-bindgen target/wasm32-unknown-unknown/release/" +
        projectName +
        ".wasm --out-dir . --target web",
      { cwd: projectPath }
    );
    if (bindgenStdout) console.log(chalk.gray(bindgenStdout));
    if (bindgenStderr) console.error(chalk.yellow(bindgenStderr));
    console.log(chalk.green("JavaScript bindings generated successfully"));
    ...
    if (!hasSchema) {
      ...
      if (schemaStdout) console.log(chalk.gray(schemaStdout));
      ...
      console.log(chalk.green("\nUsing cached schema files"));
    }

    spinner.text = "WebAssembly build completed";
    console.log(
      chalk.green("\nWebAssembly build process completed successfully")
    );

    return {
      wasmPath: path.join(projectPath, "ownable.wasm"),
      jsPath: path.join(projectPath, "ownable.js"),
    };
  } catch (error) {
    console.error(chalk.red("\nWebAssembly build failed:"), error);
