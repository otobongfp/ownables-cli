...
async function buildWasm(projectPath, spinner, fastMode = false) {
    ...
    const { stdout: buildStdout, stderr: buildStderr } = await execAsync(
      ...
      }`,
      {
        ...
        env: {
          ...
          CC: "clang",
          CXX: "clang++",
          RUSTC_WRAPPER: "sccache", // Enable sccache for faster builds
          SCCACHE_GHA_ENABLED: "true", // Enable GitHub Actions cache if available
          SCCACHE_DIR: path.join(os.homedir(), ".cache", "sccache"),
        },
      }
    );
...
async function optimizeWasm(spinner, fastMode = false) {
  if (fastMode) {
    spinner.text = chalk.cyan("Skipping optimization in fast mode");
    return;
  }
  ...
  await Promise.all([
    ...
    (async () => {
      ...
      indexHtmlContent = indexHtmlContent
        ...
        // Remove images/ prefix from quoted src attributes
        .replace(/src=["']images\//g, 'src="')
        // Ensure all src attributes are properly quoted
        .replace(/src=([^"']+)(?!["'])/g, 'src="$1"')
    ...
    const { fastMode } = await inquirer.prompt([
      {
        ...
        name: "fastMode",
        message: chalk.cyan(
          "Use fast mode? (skips optimization, faster builds)"
        ),
        default: false,
      },
    ]);
    spinner.start(chalk.cyan("Building..."));

    // Build and optimize WASM
    spinner.text = chalk.cyan("Building WebAssembly...");
    const { wasmPath, jsPath } = await buildWasm(
      process.cwd(),
      spinner,
      fastMode
    );
    await optimizeWasm(spinner, fastMode);
