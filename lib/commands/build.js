...
async function buildWasm(projectPath, spinner) {
  ...
  try {
    ...
    spinner.text = chalk.cyan("Generating JavaScript bindings...");
    ...
    if (bindgenStderr) {
      console.error("\nWASM bindgen warnings/errors:");
      console.error(bindgenStderr);
    }
    if (bindgenStdout) {
      console.log("\nWASM bindgen output:");
    ...
    const schemaResult = await execAsync("cargo run --example schema", {
      cwd: projectPath,
    });

    if (schemaResult.stderr) {
      console.error("\nSchema generation warnings/errors:");
    ...
    spinner.text = chalk.cyan("WebAssembly build complete");
    return {
      wasmPath: path.join(outDir, `${projectName}.wasm`),
...
async function createPackage(
  ...
  await fs.promises.writeFile(
    path.join(outputPath, `${metadata.name}.zip`),
    zipContent
  );
}
...
async function build() {
  ...
  try {
    ...
    const { outputDir, zipPath } = await createPackage(
      process.cwd(),
