...
async function buildWasm(projectPath, spinner) {
  ...
  try {
    ...
    process.env.RUSTFLAGS =
      "-C target-feature=+atomics,+bulk-memory,+mutable-globals -A warnings";
    ...
    try {
      ...
      if (wasmStdout) console.log(wasmStdout);
    ...
    try {
      ...
      if (bindgenStdout) console.log(bindgenStdout);
      if (bindgenStderr) console.error(chalk.yellow(bindgenStderr));
    } catch (error) {
      console.error("WASM bindgen failed:", error);
    ...
    if (!schemaExists) {
      ...
      try {
        ...
        if (schemaStdout) console.log(schemaStdout);
        ...
        if (await fs.pathExists(projectSchemaDir)) {
          ...
          console.log("No schema files found in project directory");
        }
...
async function build() {
  ...
  try {
    // Step 1: Check prerequisites and project structure
