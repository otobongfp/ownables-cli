...
async function handleMusicOwnable(projectPath, outputDir, metadata, spinner) {
  ...
  const images = await fs.readdir(imagesDir);
  const coverArt = images.find((file) => file.toLowerCase().includes("cover"));
  const background = images.find((file) =>
    file.toLowerCase().includes("background")
  );

  if (!coverArt) {
    throw new Error("No cover art image found in assets/images directory");
  }

  if (!background) {
    throw new Error("No background image found in assets/images directory");
  }
  ...
  await Promise.all([
    // Copy audio file
    fs.copy(
      path.join(audioDir, musicFile),
      path.join(outputDir, "audio", musicFile)
    ),
    // Copy and process images
    fs.copy(
      path.join(imagesDir, coverArt),
      path.join(outputDir, "images", coverArt)
    ),
    fs.copy(
      path.join(imagesDir, background),
      path.join(outputDir, "images", background)
    ),
    // Create thumbnail from cover art
    sharp(path.join(imagesDir, coverArt))
      .resize(300, 300, { fit: "inside" })
      .webp({ quality: 80 })
      .toFile(path.join(outputDir, "thumbnail.webp")),
  ]);
